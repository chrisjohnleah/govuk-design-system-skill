...
- show an [Error summary component](https://design-system.service.gov.uk/components/error-summary/) at the top of the page, and move keyboard focus to it
- show [Error message components](https://design-system.service.gov.uk/components/error-message/) next to fields with errors

Read guidance on [writing good error messages](https://design-system.service.gov.uk/components/error-message/#be-clear-and-concise).
...
Only add this sort of validation if your user research shows that, on balance, it solves more problems for users than it causes. For example, the [Character count component](https://design-system.service.gov.uk/components/character-count/) shows users an error message when they go over the character limit. Because it’s important that users do not spend time and effort writing out a response that turns out to be too long.
