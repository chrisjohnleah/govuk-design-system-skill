...
Using GOV.UK Frontend’s features means you will not have to redevelop these features yourself, and it reduces the size of the [bundled JavaScript](https://frontend.design-system.service.gov.uk/import-javascript/#import-javascript-using-a-bundler).
...
Some of the `Component`’s features are supported by static properties on the component that inherit from it. However, native static properties are not a JavaScript syntax supported by all [browsers that support `<script type="module">`](https://frontend.design-system.service.gov.uk/import-javascript/#import-javascript).
...
Use the `createAll` function to initialise your components [the same way GOV.UK Frontend components are initialised](https://frontend.design-system.service.gov.uk/import-javascript/#import-individual-components). The function will look up all the HTML elements on which a given component should be initialised and initialise a component for each of them.
